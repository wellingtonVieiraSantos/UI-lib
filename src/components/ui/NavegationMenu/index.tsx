...
const NavigationTriggerStyle =
  'group inline-flex h-8 w-max items-center justify-center gap-2 px-4 py-2 hover:bg-terciary/20 cursor-pointer data-disabled:cursor-not-allowed data-disabled:text-terciary/50 transition duration-300 rounded'
...
>(({ className, ...props }, ref) => (
  <NavigationMenuRadix.Indicator
    ref={ref}
    className={twMerge(
      'top-full z-[1] flex h-1.5 items-end justify-center overflow-hidden bg-primary',
      className
    )}
    {...props}
  >
    <div className='relative top-[60%] size-2 rotate-45 rounded-tl-sm bg-terciary/30' />
  </NavigationMenuRadix.Indicator>
))
