...
>(({ className, value, ...props }, ref) => {
  return (
    <div className='relative w-full mt-6'>
      <ProgressRadix.Root
        className={twMerge(
          `relative w-full h-3 bg-foreground-secondary rounded-full overflow-hidden`,
          className
        )}
        ref={ref}
        {...props}
      ...
      <div
        className={twMerge(`absolute -top-8 transform -translate-x-1/2 w-10 bg-foreground text-background h-5 rounded grid place-content-center
              ...
              after:border-r-4 after:border-r-transparent after:border-t-8 after:border-t-foreground transition-[left] duration-[20] ease-in-out`)}
        style={{ left: `${value || 0}%` }}
      >
        <span>{value}%</span>
      </div>
